...
/*
 * ============================================================================
 *  ConfidentialLedger v3 (candidate) — fixed 2-in / 2-out JoinSplit
 * ============================================================================
 *
 *  v3 = v2 plus two ownership constraints (audit finding H1, 2026-09-11):
 *
 *    outputOwnerPubKey[0] === recipient
 *    outputOwnerPubKey[1] === inputOwnerPubKey[0]
 *
 *  In v2 the public `recipient` was only hashed into transactionHash, so a
 *  prover could satisfy a server-authorized recipient while sending output[0]
 *  to any public key. v3 makes the authorized recipient the owner of the
 *  value-carrying output and returns change to the (first) sender, so the
 *  settlement layer's recipient eligibility check governs where value goes.
 *  Public signal order and count are unchanged (13), so the application ABI
 *  is unchanged. This is NOT an approved circuit: it still needs a freeze,
 *  independent audit and a production ceremony.
 *
 *  Privacy note: `recipient` is now the recipient's note public key. The
 *  settlement layer already sees it; publishing public signals therefore
 *  links transfers to the same recipient key. Use per-transfer recipient
 *  keys registered to the investor if that linkability is unacceptable.
 *
...
template ConfidentialLedgerJoinSplit(TREE_DEPTH) {
    ...
    // =====================================================================
    // 5b. Ownership of outputs (v3, audit H1)
    // ---------------------------------------------------------------------
    // The authorized recipient owns the value-carrying output, and change
    // returns to the sender that owns input[0]. Both owner keys are already
    // Num2Bits_strict-bound inside NoteCommitment, so equality with the
    // public field element is exact.
    // =====================================================================
    outputOwnerPubKey[0] === recipient;
    outputOwnerPubKey[1] === inputOwnerPubKey[0];
